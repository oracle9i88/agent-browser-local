# CHANGES：登录态迁移向导（Chrome → Agent）— 交付审计文档 v5

> 实现方：GLM。分支 `glm/profile-migration`，基线 `4cedd9e`。
> v4：修复第三轮审计的 Cookie 覆盖冲突 P0（冲突即中止，不覆盖、不存旧值明文）。
> v5：Codex 代码复审通过；加入知乎受限写作入口，并修复工具栏按钮被挤出可视区、
> 登录返回后 handoff 未经用户确认即自动清除两个现场缺陷。
> beta.6：把误导性的禁用状态按钮改为真正的人类控制开关：“我要接管”冻结 Agent，
> “交还 Agent”才恢复自动化。
> beta.7：现场确认小宇宙创建单集使用 `/podcast/:podcastId/episode/create`，仅把这一
> 精确贡献路由加入 allowlist，单集列表、数据与互动路径继续禁用。
> 状态：**0.3.0-beta.7 候选，待真实平台验收**。不合并到 `main`、不宣布正式完成。

## R4. 现场验收修复（beta.5）

- 工具栏改为单行弹性布局；“迁移登录态”不再把人工控制按钮挤到被正文覆盖的第二行。
- 启动门禁新增 `handoffInToolbar`，按钮必须实际位于工具栏可视范围内才允许启动。
- 从登录/验证页返回允许的贡献页面后，pending handoff 保持冻结，只有用户点击
  “交还 Agent”才能恢复自动化；不再因 URL 回到 allowlist 自动放行。
- 知乎仅注册 `https://zhuanlan.zhihu.com/write`，不开放内容列表、搜索或遍历能力。
