...
- 微信公众号：人工登录成功。直接构造编辑器 URL 时遗漏后台会话 `token`，触发重新登录；改为由用户从后台自然进入。投稿弹窗路由随后在真实页面生效：带完整会话上下文的官方编辑器已接回主受控页面。第一次真实 Snapshot 能识别封面、作者、摘要、预览、保存草稿和“发表”，但漏掉画面中可见的标题与富文本正文，因此公众号仍判定为未通过。漏项根因是微信只把标题/正文占位提示暴露为 AX 静态文本，普通控件遍历没有得到其可编辑祖先；已新增“语义提示 → 最近可编辑祖先”的 CDP 节点映射和金样测试，待一次保留 Profile 的受控重启后做真实复验。
...
第二次现场判断事故：编辑器接回后 daemon 的 `handoff` 已经为空，工具栏按旧设计会主动隐藏“我已接管”。仍要求用户点击该按钮属于未先核对状态的错误。后续版本将接管控件改为永久显示：有交接时显示可点击“我已接管”，无交接时显示禁用的“Agent 可操作”，启动中显示禁用的“Agent 未就绪”，避免内部状态与界面动作再次混淆。

## 尚未完成

- 抖音登录后的投稿页 Snapshot 与重新发现性。
- 微信公众号标题/正文语义映射的真实 Snapshot、富文本保真与重新发现性；投稿弹窗路由已在真实页面生效，但第一次 Snapshot 因漏掉标题/正文未通过。
